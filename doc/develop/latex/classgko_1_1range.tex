...
{\footnotesize template$<$typename... Accessor\+Params, typename  = std\+::enable\+\_\+if\+\_\+t$<$            sizeof...(\+Accessor\+Params) != 1 $\vert$$\vert$            !std\+::is\+\_\+same$<$                range, std\+::decay$<$detail\+::head\+\_\+t$<$\+Accessor\+Params...$>$$>$$>$\+::value$>$$>$ }\\constexpr \mbox{\hyperlink{classgko_1_1range_aede7a11a0eef2b73cabe44c9c2ad8246}{range}} (Accessor\+Params \&\&... params)
...
\end{DoxyEnumerate}\hypertarget{classgko_1_1range_autotoc_md77}{}\doxysubsubsection{Range operations}\label{classgko_1_1range_autotoc_md77}
...
\end{DoxyCode}
\hypertarget{classgko_1_1range_autotoc_md79}{}\doxysubsubsection{Caveats}\label{classgko_1_1range_autotoc_md79}
\+\_\+\+\_\+{\ttfamily mmul} is not a highly-\/optimized B\+L\+A\+S-\/3 version of the matrix multiplication.\+\_\+\+\_\+ The current design of ranges and accessors prevents that, so if you need a high-\/performance matrix multiplication, you should use one of the libraries that provide that, or implement your own (you can use pointwise range operations to help simplify that). However, range design might get improved in the future to allow efficient implementations of B\+L\+A\+S-\/3 kernels.

Aliasing the result range in {\ttfamily mmul} and {\ttfamily transpose} is not allowed. Constructs like {\ttfamily A = transpose(\+A)}, {\ttfamily A = mmul(\+A, A)}, or {\ttfamily A = mmul(\+A, A) + C} lead to undefined behavior. However, aliasing input arguments is allowed\+: {\ttfamily C = mmul(\+A, A)}, and even {\ttfamily C = mmul(\+A, A) + C} is valid code (in the last example, only pointwise operations are aliased). {\ttfamily C = mmul(A, A + C)} is not valid though.\hypertarget{classgko_1_1range_autotoc_md80}{}\doxysubsubsection{Examples}\label{classgko_1_1range_autotoc_md80}
...
\mbox{\Hypertarget{classgko_1_1range_aede7a11a0eef2b73cabe44c9c2ad8246}\label{classgko_1_1range_aede7a11a0eef2b73cabe44c9c2ad8246}} 
...
template$<$typename... Accessor\+Params, typename  = std\+::enable\+\_\+if\+\_\+t$<$            sizeof...(\+Accessor\+Params) != 1 $\vert$$\vert$            !std\+::is\+\_\+same$<$                range, std\+::decay$<$detail\+::head\+\_\+t$<$\+Accessor\+Params...$>$$>$$>$\+::value$>$$>$ \\
