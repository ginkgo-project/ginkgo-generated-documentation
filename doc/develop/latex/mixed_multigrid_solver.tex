...
This example depends on simple-\/solver.
 ...
 \label{_Mixed Precision Multigrid}%
 \label{_Thisexampleshowshowtousethemixedprecisionmultigridsolver}%
\doxysection*{This example shows how to use the mixed-\/precision multigrid solver.}

\doxysubsubsection*{In this example, we first read in a matrix from a file, then generate a right-\/hand side and an initial guess. The multigrid solver can mix different precision of Multigrid\+Level. The example features the generating time and runtime of the multigrid solver.}
...
\DoxyCodeLine{\textcolor{keyword}{using} ValueType2 = float;}
...
\DoxyCodeLine{\textcolor{keyword}{using} cg = \mbox{\hyperlink{classgko_1_1solver_1_1Cg}{gko::solver::Cg<ValueType2>}};}
\DoxyCodeLine{\textcolor{keyword}{using} ir = \mbox{\hyperlink{classgko_1_1solver_1_1Ir}{gko::solver::Ir<ValueType>}};}
\DoxyCodeLine{\textcolor{keyword}{using} ir2 = \mbox{\hyperlink{classgko_1_1solver_1_1Ir}{gko::solver::Ir<ValueType2>}};}
...
\DoxyCodeLine{\textcolor{keyword}{using} bj2 = \mbox{\hyperlink{classgko_1_1preconditioner_1_1Jacobi}{gko::preconditioner::Jacobi<ValueType2, IndexType>}};}
...
\DoxyCodeLine{\textcolor{keyword}{using} amgx\_pgm2 = \mbox{\hyperlink{classgko_1_1multigrid_1_1AmgxPgm}{gko::multigrid::AmgxPgm<ValueType2, IndexType>}};}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} \mbox{\hyperlink{namespacegko_afd46d554050c4ae90e84ea4fcd9a41f3}{gko::remove\_complex<ValueType>}} tolerance = 1e-\/12;}
\DoxyCodeLine{\textcolor{keyword}{auto} iter\_stop =}
\DoxyCodeLine{    gko::stop::Iteration::build().with\_max\_iters(100u).on(exec);}
\DoxyCodeLine{\textcolor{keyword}{auto} tol\_stop = \mbox{\hyperlink{classgko_1_1stop_1_1AbsoluteResidualNorm}{gko::stop::AbsoluteResidualNorm<ValueType>::build}}()}
...
\DoxyCodeLine{        .with\_solver(bj::build().with\_max\_block\_size(1u).on(exec))}
...
\DoxyCodeLine{        .on(exec));}
\DoxyCodeLine{\textcolor{keyword}{auto} smoother\_gen2 = \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(}
\DoxyCodeLine{    ir2::build()}
\DoxyCodeLine{        .with\_solver(bj2::build().with\_max\_block\_size(1u).on(exec))}
\DoxyCodeLine{        .with\_relaxation\_factor(0.9f)}
\DoxyCodeLine{        .with\_criteria(}
\DoxyCodeLine{            gko::stop::Iteration::build().with\_max\_iters(2u).on(exec))}
\DoxyCodeLine{        .on(exec));}
\end{DoxyCode}
...
Create Restrict\+Prolong factory


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} mg\_level\_gen = amgx\_pgm::build().with\_deterministic(\textcolor{keyword}{true}).on(exec);}
\DoxyCodeLine{\textcolor{keyword}{auto} mg\_level\_gen2 = amgx\_pgm2::build().with\_deterministic(\textcolor{keyword}{true}).on(exec);}
\end{DoxyCode}


Create Coarses\+Solver factory


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} coarsest\_solver\_gen =}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} multigrid\_gen =}
\DoxyCodeLine{    mg::build()}
\DoxyCodeLine{        .with\_max\_levels(2u)}
\DoxyCodeLine{        .with\_min\_coarse\_rows(5u)}
\DoxyCodeLine{        .with\_pre\_smoother(\mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(smoother\_gen),}
\DoxyCodeLine{                           \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(smoother\_gen2))}
\DoxyCodeLine{        .with\_post\_uses\_pre(\textcolor{keyword}{true})}
\DoxyCodeLine{        .with\_mg\_level(\mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(mg\_level\_gen), \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(mg\_level\_gen2))}
\DoxyCodeLine{        .with\_coarsest\_solver(}
\DoxyCodeLine{            \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(bj2::build().with\_max\_block\_size(1u).on(exec)))}
...
auto solver = solver\+\_\+gen-\/$>$generate(\+A);


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} solver = multigrid\_gen-\/>generate(A);}
...
\DoxyCodeLine{\mbox{\hyperlink{namespacegko_a859dc47a462721d83728d91ab7fa2148}{write}}(std::cout, \mbox{\hyperlink{namespacegko_a014ec51e675c425bb83908361f630450}{lend}}(initres));}
\DoxyCodeLine{std::cout << \textcolor{stringliteral}{"Final residual norm sqrt(r\string^T r): \(\backslash\)n"};}
\DoxyCodeLine{\mbox{\hyperlink{namespacegko_a859dc47a462721d83728d91ab7fa2148}{write}}(std::cout, \mbox{\hyperlink{namespacegko_a014ec51e675c425bb83908361f630450}{lend}}(res));}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{keyword}{auto} mg\_level\_list = solver-\/>get\_mg\_level\_list();}
\DoxyCodeLine{\textcolor{keyword}{auto} smoother\_list = solver-\/>get\_pre\_smoother\_list();}
\end{DoxyCode}


Check the Multigrid\+Level and smoother. throw error if there is mismatch


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} level0 = gko::as<amgx\_pgm>(mg\_level\_list.at(0));}
\DoxyCodeLine{\textcolor{keyword}{auto} level1 = gko::as<amgx\_pgm2>(mg\_level\_list.at(1));}
\DoxyCodeLine{\textcolor{keyword}{auto} smoother0 = gko::as<ir>(smoother\_list.at(0));}
\DoxyCodeLine{\textcolor{keyword}{auto} smoother1 = gko::as<ir2>(smoother\_list.at(1));}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Multigrid iteration count:     "}}
\DoxyCodeLine{              << logger-\/>get\_num\_iterations() << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Multigrid generation time [ms]: "}}
\DoxyCodeLine{              << static\_cast<double>(gen\_time.count()) / 1000000.0 << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Multigrid execution time [ms]: "}}
\DoxyCodeLine{              << static\_cast<double>(time.count()) / 1000000.0 << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Multigrid execution time per iteraion[ms]: "}}
...
\DoxyCodeLine{\%\%MatrixMarket matrix \mbox{\hyperlink{namespacegko_ae749a5ea11a93c1bcc9158d9a6e9fb68af1f713c9e000f5d3f280adbd124df4f5}{array}} \mbox{\hyperlink{namespacegko_a85e29870bacc4e660be5acbbf742305e}{real}} general}
\DoxyCodeLine{1 1}
\DoxyCodeLine{6.31088e-\/14}
\DoxyCodeLine{Multigrid iteration count:     9}
\DoxyCodeLine{Multigrid generation time [ms]: 3.35361}
\DoxyCodeLine{Multigrid execution time [ms]: 10.048}
\DoxyCodeLine{Multigrid execution time per iteraion[ms]: 1.11644}
\end{DoxyCode}
