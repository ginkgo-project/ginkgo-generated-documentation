...
\item {\ttfamily -\/D\+G\+I\+N\+K\+G\+O\+\_\+\+E\+N\+A\+B\+L\+E\+\_\+\+H\+A\+LF=\{ON, O\+FF\}} enable half precision support in Ginkgo, default is {\ttfamily ON}. It is {\ttfamily O\+FF} when the compiler is M\+S\+VC or M\+S\+Y\+S2 system. If compiling is done with the C\+U\+DA backend before C\+U\+DA 12.\+2, we only support half precision after compute capability 5.\+3. C\+U\+DA 12.\+2+ compilers waive the compute capbility limitation.
\item {\ttfamily -\/D\+G\+I\+N\+K\+G\+O\+\_\+\+E\+N\+A\+B\+L\+E\+\_\+\+B\+F\+L\+O\+A\+T16=\{ON, O\+FF\}} enable half precision support in Ginkgo, default is {\ttfamily O\+FF}. It is {\ttfamily O\+FF} when the compiler is M\+S\+VC or M\+S\+Y\+S2 system. If compiling is done with the C\+U\+DA backend before C\+U\+DA 12.\+2, we only support bfloat16 precision after compute capability 8.\+0. C\+U\+DA 12.\+2+ compilers waive the compute capbility limitation.
...
\item {\ttfamily -\/D\+C\+M\+A\+K\+E\+\_\+\+H\+I\+P\+\_\+\+A\+R\+C\+H\+I\+T\+E\+C\+T\+U\+R\+ES=\char`\"{}gpuarch1;gpuarch2\char`\"{}} the A\+M\+D\+G\+PU targets to be passed to the compiler. If empty, compiler chooses based on the available G\+P\+Us.
...
Ginkgo relies on C\+Make to decide which compiler to use for H\+IP. To choose {\ttfamily nvcc} instead of the default R\+O\+Cm {\ttfamily clang++}, set the corresponding environment variable\+: 
\begin{DoxyCode}{0}
\DoxyCodeLine{export HIPCXX=nvcc}
\end{DoxyCode}
 Note that this option is currently not being tested in our CI pipelines.\hypertarget{install_ginkgo_autotoc_md5}{}\doxysubsection{Third party libraries and packages}\label{install_ginkgo_autotoc_md5}
...
For convenience, the options {\ttfamily G\+I\+N\+K\+G\+O\+\_\+\+I\+N\+S\+T\+A\+L\+L\+\_\+\+R\+P\+A\+TH\mbox{[}\+\_\+.$\ast$\mbox{]}} can be used to bind the installed Ginkgo shared libraries to the path of its dependencies.\hypertarget{install_ginkgo_autotoc_md6}{}\doxysubsection{Installing Ginkgo}\label{install_ginkgo_autotoc_md6}
