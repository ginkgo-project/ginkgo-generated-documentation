...
\begin{DoxyItemize}
\item cmd\+: {\ttfamily set P\+A\+TH=$<$P\+R\+O\+J\+E\+C\+T\+\_\+\+B\+I\+N\+A\+R\+Y\+\_\+\+D\+I\+R/\+G\+I\+N\+K\+G\+O\+\_\+\+W\+I\+N\+D\+O\+W\+S\+\_\+\+S\+H\+A\+R\+E\+D\+\_\+\+L\+I\+B\+R\+A\+R\+Y\+\_\+\+R\+E\+L\+P\+A\+TH$>$;P\+A\+TH\%}
\item powershell\+: {\ttfamily \begin{DoxyParagraph}{env}
A\+TH="$<$P\+R\+O\+J\+E\+C\+T\+\_\+\+B\+I\+N\+A\+R\+Y\+\_\+\+D\+I\+R/\+G\+I\+N\+K\+G\+O\+\_\+\+W\+I\+N\+D\+O\+W\+S\+\_\+\+S\+H\+A\+R\+E\+D\+\_\+\+L\+I\+B\+R\+A\+R\+Y\+\_\+\+R\+E\+L\+P\+A\+TH$>$;
\end{DoxyParagraph}
env\+:P\+A\+TH"}
\end{DoxyItemize}
...
\begin{DoxyEnumerate}
\item {\ttfamily bigobj} issue\+: encountering {\ttfamily too many sections} needs the compilation flags {\ttfamily /bigobj} or {\ttfamily -\/Wa,-\/mbig-\/obj}
...
\begin{DoxyItemize}
\item {\ttfamily cmake -\/D\+C\+M\+A\+K\+E\+\_\+\+C\+X\+X\+\_\+\+F\+L\+A\+GS=/bigobj $<$other parameters$>$ $<$source\+\_\+folder$>$} which might overwrite the default settings.
\item add {\ttfamily /bigobj} into the environment variable {\ttfamily C\+X\+X\+F\+L\+A\+GS} (only available in the first cmake configuration)
\begin{DoxyItemize}
\item cmd\+: {\ttfamily set C\+X\+X\+F\+L\+A\+GS=/bigobj}
\item powershell\+: {\ttfamily \$env\+:C\+X\+X\+F\+L\+A\+GS=/bigobj}
\end{DoxyItemize}
\end{DoxyItemize}
...
\item powershell\+: {\ttfamily \$env\+:C\+X\+X\+F\+L\+A\+GS=\char`\"{}-\/\+Wa,-\/mbig-\/obj\char`\"{}}
\end{DoxyItemize}
\item {\ttfamily cmake -\/D\+C\+M\+A\+K\+E\+\_\+\+C\+X\+X\+\_\+\+F\+L\+A\+GS=-\/Wa,-\/mbig-\/obj $<$other parameters$>$ $<$source\+\_\+folder$>$}, which might overwrite the default settings.
\end{DoxyItemize}
\item {\ttfamily ld} issue (If building Ginkgo as static library, this is not needed)
...
\end{DoxyEnumerate}
\end{DoxyItemize}
\item Possible issue when switching static/shared of Ginkgo with M\+S\+VC in the same build directory\+:\textbackslash{} If an issue occurs from mixing M\+D/\+MT runtime library when enabling {\ttfamily G\+I\+N\+K\+G\+O\+\_\+\+B\+U\+I\+L\+D\+\_\+\+B\+E\+N\+C\+H\+M\+A\+R\+KS}, it means the third-\/party flags are not updated correctly. To update the third party flags, turn off {\ttfamily G\+I\+N\+K\+G\+O\+\_\+\+S\+K\+I\+P\+\_\+\+D\+E\+P\+E\+N\+D\+E\+N\+C\+Y\+\_\+\+U\+P\+D\+A\+TE} ({\ttfamily -\/D\+G\+I\+N\+K\+G\+O\+\_\+\+S\+K\+I\+P\+\_\+\+D\+E\+P\+E\+N\+D\+E\+N\+C\+Y\+\_\+\+U\+P\+D\+A\+TE=O\+FF}).
...
\item roc\+R\+A\+ND,
\item roc\+Thrust.
\end{DoxyItemize}\hypertarget{install_ginkgo_autotoc_md4}{}\doxysubsubsection{Changing the paths to search for H\+I\+P and other packages}\label{install_ginkgo_autotoc_md4}
...
When using {\ttfamily H\+I\+P\+\_\+\+P\+L\+A\+T\+F\+O\+RM=hcc} (or {\ttfamily amd}), note that two {\ttfamily H\+IP} compilers can be set\+: the old {\ttfamily hcc}, or since R\+O\+Cm 3.\+5, {\ttfamily clang}. Ginkgo is only compatible with the {\ttfamily clang} based installations. Although this setting should be automatically done, it is also possible to manually set the {\ttfamily H\+IP} compiler to {\ttfamily clang}\+: 
\begin{DoxyCode}{0}
\DoxyCodeLine{export HIP\_COMPILER=clang}
\end{DoxyCode}
...
Usually, Ginkgo uses the internal version of each package. For each of the packages {\ttfamily G\+T\+E\+ST}, {\ttfamily G\+F\+L\+A\+GS}, {\ttfamily R\+A\+P\+I\+D\+J\+S\+ON} and {\ttfamily C\+AS}, it is possible to force Ginkgo to try to use an external version of a package. For this, Ginkgo provides two ways to find packages. To rely on the C\+Make {\ttfamily find\+\_\+package} command, use the C\+Make option {\ttfamily -\/D\+G\+I\+N\+K\+G\+O\+\_\+\+U\+S\+E\+\_\+\+E\+X\+T\+E\+R\+N\+A\+L\+\_\+$<$package$>$=ON}. {\ttfamily H\+W\+L\+OC} works the opposite way, Ginkgo always looks for the system\textquotesingle{}s {\ttfamily hwloc} first. In addition, when installing Ginkgo, {\ttfamily hwloc} will be installed as well into the Ginkgo directory if there were no system {\ttfamily hwloc} detected.
...
When applicable (e.\+g. for {\ttfamily G\+Test} libraries), a {\ttfamily ;} separated list can be given to the {\ttfamily T\+P\+L\+\_\+$<$package$>$\+\_\+\{L\+I\+B\+R\+A\+R\+I\+E\+S$\vert$\+I\+N\+C\+L\+U\+D\+E\+\_\+\+D\+I\+RS\}} variables.

Note that for convenience, the options {\ttfamily G\+I\+N\+K\+G\+O\+\_\+\+I\+N\+S\+T\+A\+L\+L\+\_\+\+R\+P\+A\+TH\mbox{[}\+\_\+.$\ast$\mbox{]}} can be used to make bind the Ginkgo shared libraries to the path of its dependencies.\hypertarget{install_ginkgo_autotoc_md8}{}\doxysubsection{Installing Ginkgo}\label{install_ginkgo_autotoc_md8}
