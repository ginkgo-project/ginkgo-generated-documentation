...
This example depends on simple-\/solver.
 ...
 \label{_preconditioner_export_Intro}%
 \label{_preconditioner_export_Introduction}%
\doxysection*{Introduction}

\label{_preconditioner_export_Abouttheexample}%
\doxysubsubsection*{About the example }

This example shows how to explicitly generate and store preconditioners for a given matrix. It can also be used to inspect and debug the preconditioner generation. \label{_preconditioner_export_CommProg}%
...
\DoxyCodeLine{    \mbox{\hyperlink{namespacegko_a577a2f0345ee607f64277b3364b6431b}{gko::write}}(stream, mtx, \mbox{\hyperlink{namespacegko_ae749a5ea11a93c1bcc9158d9a6e9fb68af5d7aa3ba4929cc12dc51a92c59fabd3}{gko::layout\_type::coordinate}});}
\DoxyCodeLine{\}}
\DoxyCodeLine{}
\DoxyCodeLine{}
...
\DoxyCodeLine{        std::cerr << \textcolor{stringliteral}{"Error: "} << err.what() << \textcolor{charliteral}{'\(\backslash\)n'};}
...
\DoxyCodeLine{\}}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{keyword}{using} LowerIsai = \mbox{\hyperlink{classgko_1_1preconditioner_1_1Isai}{gko::preconditioner::LowerIsai<>}};}
\DoxyCodeLine{\textcolor{keyword}{using} UpperIsai = \mbox{\hyperlink{classgko_1_1preconditioner_1_1Isai}{gko::preconditioner::UpperIsai<>}};}
\end{DoxyCode}
...
\DoxyCodeLine{        \mbox{\hyperlink{classgko_1_1preconditioner_1_1Ilu}{gko::preconditioner::Ilu<>::build}}()}
\DoxyCodeLine{            .with\_factorization(fact\_factory)}
\DoxyCodeLine{            .with\_l\_solver(}
\DoxyCodeLine{                LowerIsai::build().with\_sparsity\_power(sparsity\_power))}
\DoxyCodeLine{            .with\_u\_solver(}
\DoxyCodeLine{                UpperIsai::build().with\_sparsity\_power(sparsity\_power))}
...
\DoxyCodeLine{    output(gko::as<LowerIsai>(ilu\_isai-\/>get\_l\_solver())}
\DoxyCodeLine{               -\/>get\_approximate\_inverse(),}
\DoxyCodeLine{           matrix + \textcolor{stringliteral}{".ilu-\/isai"} + output\_suffix + \textcolor{stringliteral}{"-\/l"});}
\DoxyCodeLine{    output(gko::as<UpperIsai>(ilu\_isai-\/>get\_u\_solver())}
\DoxyCodeLine{               -\/>get\_approximate\_inverse(),}
...
\DoxyCodeLine{\}}
\end{DoxyCode}
 \label{_preconditioner_export_Results}%
\doxysection*{Results}
...
\label{_preconditioner_export_Commentsaboutprogramminganddebugging}%
\doxysubsubsection*{Comments about programming and debugging }

\label{_preconditioner_export_PlainProg}%
