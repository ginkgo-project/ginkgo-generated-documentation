...
To compile the benchmarks, the flag {\ttfamily -\/D\+G\+I\+N\+K\+G\+O\+\_\+\+B\+U\+I\+L\+D\+\_\+\+B\+E\+N\+C\+H\+M\+A\+R\+KS=ON} has to be set during the {\ttfamily cmake} step. In addition, the \href{https://github.com/ginkgo-project/ssget}{\texttt{ {\ttfamily ssget} command-\/line utility}} can be installed on the system to load matrices. The purpose of this file is to explain in detail the capacities of this benchmarking suite as well as how to properly setup everything.

There are two ways to benchmark Ginkgo. When compiling the benchmark suite, executables are generated for collecting matrix statistics, running sparse-\/matrix vector product, solvers (possibly distributed) benchmarks. Thus, one can run the needed executable for benchmarking \href{\#3-running-benchmarks-manually}{\texttt{ {\bfseries{manually}}}}. Another way to run (almost) \href{\#4-benchmarking-with-make-benchmark-or-run_all_benchmarkssh}{\texttt{ {\bfseries{all benchmarks}}}} is through the convenience script {\ttfamily run\+\_\+all\+\_\+benchmarks.\+sh}, but not all features are exposed through this tool!

Here is a short description of the content of this file\+:
\begin{DoxyEnumerate}
\item \href{\#1-ginkgo-setup-and-best-practice-guidelines}{\texttt{ Ginkgo setup and best practice guidelines}}
\item \href{\#2-using-ssget-to-fetch-the-matrices}{\texttt{ Using ssget to fetch the matrices}}
\item \href{\#3-running-benchmarks-manually}{\texttt{ Running benchmarks manually}}
\item \href{\#4-benchmarking-with-make-benchmark-or-run_all_benchmarkssh}{\texttt{ Benchmarking with {\ttfamily make benchmark} or {\ttfamily run\+\_\+all\+\_\+benchmarks.\+sh}}}
\item \href{\#5-publishing-the-results-on-github-and-analyze-the-results-with-the-gpe-optional}{\texttt{ Publishing the results on Github and analyze the results with the G\+PE (optional)}}
\item \href{\#6-detailed-performance-analysis-and-debugging}{\texttt{ Detailed performance analysis and debugging}}
\item \href{\#7-available-benchmark-options}{\texttt{ Available benchmark options}}
\end{DoxyEnumerate}\hypertarget{benchmarking_ginkgo_autotoc_md11}{}\doxysubsection{1\+: Ginkgo setup and best practice guidelines}\label{benchmarking_ginkgo_autotoc_md11}
...
\end{DoxyEnumerate}\hypertarget{benchmarking_ginkgo_autotoc_md12}{}\doxysubsection{2\+: Using ssget to fetch the matrices}\label{benchmarking_ginkgo_autotoc_md12}
To benchmark {\ttfamily ginkgo}, matrices need to be provided as input in the {\ttfamily Matrix Market} format. A convenient way is to run benchmark with the \href{https://sparse.tamu.edu/}{\texttt{ Suite\+Sparse matrix collection}}. A helper tool, the \href{https://github.com/ginkgo-project/ssget}{\texttt{ {\ttfamily ssget} command-\/line utility}} can be used to facilitate downloading and extracting matrices from the suitesparse collection. When running the benchmarks with the helper script {\ttfamily run\+\_\+all\+\_\+benchmarks.\+sh} (or calling {\ttfamily make benchmark}), the {\ttfamily ssget} tool is required.

To install {\ttfamily ssget}, access the repository and copy the file {\ttfamily ssget} into a directory present in your {\ttfamily P\+A\+TH} variable as per the tool\textquotesingle{}s {\ttfamily R\+E\+A\+D\+M\+E.\+md} instructions. The tool can be installed either in a global system path or a local directory such as {\ttfamily \$\+H\+O\+ME/.local/bin}. After installing the tool, it is important to review the {\ttfamily ssget} script and configure as needed the variable {\ttfamily A\+R\+C\+H\+I\+V\+E\+\_\+\+L\+O\+C\+A\+T\+I\+ON} on line 39. This is where the matrices will be stored into. As of \href{https://github.com/ginkgo-project/ssget/pull/7}{\texttt{ PR\#7}}, one can also configure the archive location using the {\ttfamily -\/a} option without modifying the script itself. You can also use this script without installing it.
...
For extracting the matrices, {\ttfamily ssget -\/f -\/i \$\{i\}} can be used.\hypertarget{benchmarking_ginkgo_autotoc_md13}{}\doxysubsection{3\+: Running benchmarks manually}\label{benchmarking_ginkgo_autotoc_md13}
As mentioned above, one way to run benchmarks is through the executable of a certain needed type of benchmarking.

When compiling Ginkgo with the flag {\ttfamily -\/D\+G\+I\+N\+K\+G\+O\+\_\+\+B\+U\+I\+L\+D\+\_\+\+B\+E\+N\+C\+H\+M\+A\+R\+KS=ON}, a suite of executables will be generated depending on the C\+Make configuration. These executables are the backbone of the benchmarking suite. Note that all of these executables describe the available options and the required input format when running them with the {\ttfamily -\/-\/help} option. All executables have multiple variants depending on the precision, by default {\ttfamily double} precision is used for the type of values, but variants with {\ttfamily single} and {\ttfamily complex} (single and double) value types are also available. Here is a non exhaustive list of the available benchmarks\+:


\begin{DoxyItemize}
\item {\ttfamily blas/blas}\+: supports benchmarking many of Ginkgo\textquotesingle{}s B\+L\+AS operations\+: dot products, axpy, copy, etc.
\item {\ttfamily conversion/conversion}\+: conversion between matrix formats.
\item {\ttfamily matrix\+\_\+generator/matrix\+\_\+generator}\+: mostly allows generating block diagonal matrices (to benchmark the block-\/jacobi preconditioner).
\item {\ttfamily matrix\+\_\+statistics/matrix\+\_\+statistics}\+: computes size and other matrix statistics (such as variance, load imbalance, ...).
\item {\ttfamily preconditioner/preconditioner}\+: benchmarks most Ginkgo preconditioner.
\item {\ttfamily solver/solver}\+: benchmark most of Ginkgo\textquotesingle{}s solvers in a non distributed setting.
\item {\ttfamily sparse\+\_\+blas/sparse\+\_\+blas}\+: benchmarks Sparse B\+L\+AS operations, such as Sp\+G\+E\+MM, Sp\+G\+E\+AM, transpose.
\item {\ttfamily spmv/spmv}\+: benchmarks Ginkgo\textquotesingle{}s matrix formats (Sparse-\/\+Matrix Vector product).
\end{DoxyItemize}

Optionally when compiling with M\+PI support\+:
\begin{DoxyItemize}
\item {\ttfamily blas/distributed/multi\+\_\+vector}\+: measures B\+L\+AS performance on (distributed) multi-\/vectors.
\item {\ttfamily solver/distributed/solver}\+: distributed solver benchmarks.
\item {\ttfamily spmv/distributed/spmv}\+: distributed matrix Sparse-\/\+Matrix Vector (Sp\+MV) product benchmark.
\end{DoxyItemize}

All benchmarks require input data as in a {\ttfamily J\+S\+ON} format. The json file has to consist of exactly one array, and within that array the test cases are defined. The exact syntax can change between executables, the {\ttfamily -\/-\/help} option will explain the necessary {\ttfamily J\+S\+ON} input format. For example for the {\ttfamily spmv} benchmark case, and many other benchmarks the following minimal input should be provided\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{[}
\DoxyCodeLine{  \{}
\DoxyCodeLine{    "filename": "path/to/your/matrix",}
\DoxyCodeLine{    "rhs": "path/to/your/rhs"}
\DoxyCodeLine{  \},}
\DoxyCodeLine{  \{ ... \}}
\DoxyCodeLine{]}
\end{DoxyCode}
...
Some benchmarks require some extra fields. For example, the {\bfseries{solver benchmarks}} requires the field {\ttfamily \char`\"{}optimal\char`\"{}\+: \{\char`\"{}spmv\char`\"{}\+: \char`\"{}matrix format (such as csr)\char`\"{}\}}. This is automatically populated when running the {\ttfamily spmv} benchmark which finds the optimal (fastest) format among all requested formats.

The {\ttfamily \char`\"{}rhs\char`\"{}} field (right-\/hand side) is generally optional for the {\bfseries{solver benchmarks}} if you generate the R\+HS with the {\ttfamily -\/rhs\+\_\+generation} flag (use {\ttfamily -\/-\/help} with benchmark executables if this is unclear).

After writing the necessary data in a J\+S\+ON file, the benchmark can be called by passing in the input via stdin, i.\+e.


\begin{DoxyCode}{0}
\DoxyCodeLine{./solver < input.json}
\end{DoxyCode}


The output of our benchmarks is again J\+S\+ON, and it is printed to stdout, while our status messages are printed to stderr. So, the output can be stored with


\begin{DoxyCode}{0}
\DoxyCodeLine{./solver < input.json > output.json}
\end{DoxyCode}


Note that in most cases, the J\+S\+ON output by our benchmarks is compatible with other benchmarks, therefore it is possible to first call the {\ttfamily spmv} benchmark, use the resulting output J\+S\+ON as input to the {\ttfamily solver} benchmark, and finally use the resulting solver J\+S\+ON output as input to the {\ttfamily preconditioner} benchmark.\hypertarget{benchmarking_ginkgo_autotoc_md14}{}\doxysubsection{4\+: Benchmarking with $<$tt$>$make benchmark$<$/tt$>$ or $<$tt$>$run\+\_\+all\+\_\+benchmarks.\+sh$<$/tt$>$}\label{benchmarking_ginkgo_autotoc_md14}
...
The benchmark suite can take a number of configuration parameters. Benchmarks can be run only for {\ttfamily sparse matrix vector products (spmv)}, for full solvers (with or without preconditioners), or for preconditioners only when supported. The benchmark suite also allows to target a sub-\/part of the Suite\+Sparse matrix collection. For details, see the \href{\#7-available-benchmark-options}{\texttt{ available benchmark options}}. Here are the most important options\+:
\begin{DoxyItemize}
\item {\ttfamily B\+E\+N\+C\+H\+M\+A\+RK=\{spmv, solver, preconditioner\}} -\/ allows to select the type of benchmark to be run.
\item {\ttfamily E\+X\+E\+C\+U\+T\+OR=\{reference,cuda,hip,omp,dpcpp\}} -\/ select the executor and platform the benchmarks should be run on.
...
\end{DoxyItemize}\hypertarget{benchmarking_ginkgo_autotoc_md15}{}\doxysubsection{5\+: Publishing the results on Github and analyze the results with the G\+P\+E (optional)}\label{benchmarking_ginkgo_autotoc_md15}
...
\end{DoxyEnumerate}\hypertarget{benchmarking_ginkgo_autotoc_md16}{}\doxysubsection{6\+: Detailed performance analysis and debugging}\label{benchmarking_ginkgo_autotoc_md16}
...
These features are implemented thanks to the loggers located in the file {\ttfamily \$\{ginkgo\+\_\+src\+\_\+dir\}/benchmark/utils/loggers.hpp}. Ginkgo possesses hooks at all important code location points which can be inspected thanks to the logger. In this fashion, it is easy to use these loggers also for tracking memory allocation sizes and other important library aspects.\hypertarget{benchmarking_ginkgo_autotoc_md17}{}\doxysubsection{7\+: Available benchmark options}\label{benchmarking_ginkgo_autotoc_md17}
...
\begin{DoxyItemize}
\item {\ttfamily B\+E\+N\+C\+H\+M\+A\+RK=\{spmv, solver, preconditioner\}} -\/ allows to select the type of benchmark to be run. Default is {\ttfamily spmv}.
\begin{DoxyItemize}
...
\end{DoxyItemize}
\item {\ttfamily E\+X\+E\+C\+U\+T\+OR=\{reference,cuda,hip,omp,dpcpp\}} -\/ select the executor and platform the benchmarks should be run on. Default is {\ttfamily cuda}.
...
\item {\ttfamily S\+O\+L\+V\+E\+RS=\{bicgstab,bicg,cg,cgs,fcg,pipe\+\_\+cg,gmres,cb\+\_\+gmres\+\_\+\{keep,reduce1,reduce2,integer,ireduce1,ireduce2\},lower\+\_\+trs,upper\+\_\+trs\}}
\begin{DoxyItemize}
...
\item {\ttfamily S\+O\+L\+V\+E\+R\+S\+\_\+\+M\+A\+X\+\_\+\+I\+T\+E\+R\+A\+T\+I\+O\+NS=$<$number$>$} -\/ the maximum number of iterations with which a solver should be run. The default is {\ttfamily 10000}.
...
\item {\ttfamily D\+E\+T\+A\+I\+L\+ED=\{0,1\}} -\/ selects whether detailed benchmarks should be run. This generally provides extra, verbose information at the cost of one or more extra benchmark runs. It can be either {\ttfamily 0} (off) or {\ttfamily 1} (on).
