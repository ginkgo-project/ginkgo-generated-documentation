...
This example depends on distributed-\/solver.
 ...
 \label{_distributed_multigrid_preconditioned_solver_Intro}%
 \label{_distributed_multigrid_preconditioned_solver_Introduction}%
\doxysection*{Introduction}

This distributed multigrid preconditioned solver example should help you understand customizing Ginkgo multigrid in a distributed setting. The example will solve a simple 1D Laplace equation where the system can be distributed row-\/wise to multiple processes. Note. Because the stencil for the discretized Laplacian is configured with equal weight, the coarsening method does not perform well on this kind of problem. To run the solver with multiple processes, use {\ttfamily mpirun -\/n N\+U\+M\+\_\+\+P\+R\+O\+CS ./distributed-\/solver \mbox{[}executor\mbox{]} \mbox{[}num\+\_\+grid\+\_\+points\mbox{]} \mbox{[}num\+\_\+iterations\mbox{]}}.

If you are using G\+PU devices, please make sure that you run this example with at most as many processes as you have G\+PU devices available. \label{_distributed_multigrid_preconditioned_solver_CommProg}%
 \doxysection*{The commented program}

\label{_distributed_multigrid_preconditioned_solver_Includefiles}%
...
\label{_distributed_multigrid_preconditioned_solver_InitializetheMPIenvironment}%
...
\label{_distributed_multigrid_preconditioned_solver_TypeDefinitions}%
...
\DoxyCodeLine{\textcolor{keyword}{using} pgm = \mbox{\hyperlink{classgko_1_1multigrid_1_1Pgm}{gko::multigrid::Pgm<ValueType, LocalIndexType>}};}
\DoxyCodeLine{\textcolor{keyword}{using} uc = \mbox{\hyperlink{classgko_1_1multigrid_1_1UniformCoarsening}{gko::multigrid::UniformCoarsening<ValueType, LocalIndexType>}};}
\end{DoxyCode}
...
\label{_distributed_multigrid_preconditioned_solver_UserInputHandling}%
...
\DoxyCodeLine{                  << \textcolor{stringliteral}{" [executor] [grid\_side] [num\_iterations]"}}
\DoxyCodeLine{                  << \textcolor{stringliteral}{" [mg\_type=pgm|uc-\/agg|uc-\/inj] [stencil=1d|2d]"}}
...
\DoxyCodeLine{\textcolor{keyword}{const} std::string mg\_type = argc >= 5 ? argv[4] : \textcolor{stringliteral}{"pgm"};}
\DoxyCodeLine{\textcolor{keyword}{const} std::string stencil = argc >= 6 ? argv[5] : \textcolor{stringliteral}{"1d"};}
\DoxyCodeLine{}
...
\label{_distributed_multigrid_preconditioned_solver_CreatingtheDistributedMatrixandVectors}%
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keywordtype}{bool} is\_2d = (stencil == \textcolor{stringliteral}{"2d"});}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keyword}{auto} num\_rows = is\_2d ? grid\_dim * grid\_dim : grid\_dim;}
...
Assemble the matrix using either a 3-\/pt 1D Laplacian stencil or a 5-\/pt 2D Laplacian stencil on a grid\+\_\+dim x grid\+\_\+dim grid (row-\/major). Rows are partitioned across ranks; in 2D, rows near a rank boundary carry cross-\/rank non-\/local entries via the i ± grid\+\_\+dim neighbours.
...
\DoxyCodeLine{\textcolor{keywordflow}{if} (is\_2d) \{}
\DoxyCodeLine{    \textcolor{keyword}{const} \textcolor{keyword}{auto} N = static\_cast<GlobalIndexType>(grid\_dim);}
\DoxyCodeLine{    \textcolor{keywordflow}{for} (GlobalIndexType i = range\_start; i < range\_end; i++) \{}
\DoxyCodeLine{        \textcolor{keyword}{const} \textcolor{keyword}{auto} r = i / N;}
\DoxyCodeLine{        \textcolor{keyword}{const} \textcolor{keyword}{auto} c = i \% N;}
\DoxyCodeLine{        \textcolor{keywordflow}{if} (r > 0) \{}
\DoxyCodeLine{            A\_data.\mbox{\hyperlink{structgko_1_1matrix__data_a4f6be92270fa96bccdc502c83d248dc0}{nonzeros}}.emplace\_back(i, i -\/ N, -\/1);}
\DoxyCodeLine{        \}}
\DoxyCodeLine{        \textcolor{keywordflow}{if} (c > 0) \{}
...
\DoxyCodeLine{        \}}
\DoxyCodeLine{        A\_data.\mbox{\hyperlink{structgko_1_1matrix__data_a4f6be92270fa96bccdc502c83d248dc0}{nonzeros}}.emplace\_back(i, i, 4);}
\DoxyCodeLine{        \textcolor{keywordflow}{if} (c < N -\/ 1) \{}
\DoxyCodeLine{            A\_data.\mbox{\hyperlink{structgko_1_1matrix__data_a4f6be92270fa96bccdc502c83d248dc0}{nonzeros}}.emplace\_back(i, i + 1, -\/1);}
\DoxyCodeLine{        \}}
\DoxyCodeLine{        \textcolor{keywordflow}{if} (r < N -\/ 1) \{}
\DoxyCodeLine{            A\_data.\mbox{\hyperlink{structgko_1_1matrix__data_a4f6be92270fa96bccdc502c83d248dc0}{nonzeros}}.emplace\_back(i, i + N, -\/1);}
\DoxyCodeLine{        \}}
...
\DoxyCodeLine{        \textcolor{keywordflow}{if} (i < static\_cast<GlobalIndexType>(grid\_dim) -\/ 1) \{}
\DoxyCodeLine{            A\_data.\mbox{\hyperlink{structgko_1_1matrix__data_a4f6be92270fa96bccdc502c83d248dc0}{nonzeros}}.emplace\_back(i, i + 1, -\/1);}
\DoxyCodeLine{        \}}
\DoxyCodeLine{        b\_data.\mbox{\hyperlink{structgko_1_1matrix__data_a4f6be92270fa96bccdc502c83d248dc0}{nonzeros}}.emplace\_back(i, 0, std::sin(i * 0.01));}
\DoxyCodeLine{        x\_data.\mbox{\hyperlink{structgko_1_1matrix__data_a4f6be92270fa96bccdc502c83d248dc0}{nonzeros}}.emplace\_back(i, 0, gko::zero<ValueType>());}
...
\label{_distributed_multigrid_preconditioned_solver_SolvetheDistributedSystem}%
 \doxysubsubsection*{Solve the Distributed System}

Generate the solver

Setup the multigrid factory with customized smoother and coarse solver Because Block\+Jacobi does not support distributed matrix, we need wrap it in Schwarz.


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} schwarz\_bj\_factory =}
\DoxyCodeLine{    \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(schwarz::build().with\_local\_solver(bj::build()).on(exec));}
\DoxyCodeLine{\textcolor{keyword}{auto} smoother\_factory = \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(\mbox{\hyperlink{namespacegko_1_1solver_ac565c4b95ad0c67492073d993a9170cc}{gko::solver::build\_smoother}}(}
\DoxyCodeLine{    schwarz\_bj\_factory, 2u, static\_cast<ValueType>(0.9)));}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} coarsest\_factory = \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(}
\DoxyCodeLine{    solver::build()}
\DoxyCodeLine{        .with\_criteria(gko::stop::Iteration::build().with\_max\_iters(4u))}
\DoxyCodeLine{        .on(exec));}
\end{DoxyCode}


The multigrid preconditioner uses the Schwarz Jacobi as smoother and Cg as coarse solver. The mg\+\_\+level can be P\+GM or Uniform\+Coarsening (aggregation-\/ or injection-\/style), selected by the C\+LI argument.


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} make\_mg\_factory = [\&]() \{}
\DoxyCodeLine{    \textcolor{keyword}{auto} base = mg::build()}
\DoxyCodeLine{                    .with\_pre\_smoother(smoother\_factory)}
\DoxyCodeLine{                    .with\_coarsest\_solver(coarsest\_factory)}
\DoxyCodeLine{                    .with\_criteria(}
\DoxyCodeLine{                        gko::stop::Iteration::build().with\_max\_iters(1u));}
\DoxyCodeLine{    \textcolor{keywordflow}{if} (mg\_type == \textcolor{stringliteral}{"uc-\/agg"}) \{}
\DoxyCodeLine{        \textcolor{keywordflow}{return} base}
\DoxyCodeLine{            .with\_mg\_level(}
\DoxyCodeLine{                uc::build().with\_aggregation(\textcolor{keyword}{true}).with\_coarse\_skip(2))}
\DoxyCodeLine{            .on(exec);}
\DoxyCodeLine{    \} \textcolor{keywordflow}{else} \textcolor{keywordflow}{if} (mg\_type == \textcolor{stringliteral}{"uc-\/inj"}) \{}
\DoxyCodeLine{        \textcolor{keywordflow}{return} base}
\DoxyCodeLine{            .with\_mg\_level(}
\DoxyCodeLine{                uc::build().with\_aggregation(\textcolor{keyword}{false}).with\_coarse\_skip(2))}
\DoxyCodeLine{            .on(exec);}
\DoxyCodeLine{    \} \textcolor{keywordflow}{else} \{}
\DoxyCodeLine{        \textcolor{keywordflow}{return} base.with\_mg\_level(pgm::build().with\_deterministic(\textcolor{keyword}{true}))}
\DoxyCodeLine{            .on(exec);}
\DoxyCodeLine{    \}}
\DoxyCodeLine{\};}
\DoxyCodeLine{\textcolor{keyword}{auto} mg\_factory = \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(make\_mg\_factory());}
\end{DoxyCode}
...
\DoxyCodeLine{                .with\_preconditioner(mg\_factory)}
\DoxyCodeLine{                .with\_criteria(}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} res\_norm = \mbox{\hyperlink{namespacegko_adc9a73d088d88d5dfdaf63aff9e96012}{gko::clone}}(exec-\/>get\_master(),}
\DoxyCodeLine{                           gko::as<vec>(logger-\/>get\_residual\_norm()));}
\end{DoxyCode}


\label{_distributed_multigrid_preconditioned_solver_PrintingResults}%
...
\DoxyCodeLine{\}}
\end{DoxyCode}
 \label{_distributed_multigrid_preconditioned_solver_Results}%
\doxysection*{Results}

This is the expected output for {\ttfamily mpirun -\/n 4 ./distributed-\/multigrid-\/preconditioned-\/solver}\+:
...
\DoxyCodeLine{Final Res norm: 1.61045e-\/08}
\DoxyCodeLine{Iteration count: 18}
\DoxyCodeLine{Init time: 0.000117699}
\DoxyCodeLine{Read time: 0.000522518}
\DoxyCodeLine{Solver generate time: 0.000430548}
\DoxyCodeLine{Solver apply time: 0.00183804}
\DoxyCodeLine{Total time: 0.00279111}
\end{DoxyCode}


The timings may vary depending on the machine. \label{_distributed_multigrid_preconditioned_solver_PlainProg}%
