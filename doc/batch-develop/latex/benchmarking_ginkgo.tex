...
\end{DoxyItemize}\hypertarget{benchmarking_ginkgo_autotoc_md15}{}\doxysubsubsection{3.\+1\+: Benchmarking experimental batched functionality}\label{benchmarking_ginkgo_autotoc_md15}
\hypertarget{benchmarking_ginkgo_autotoc_md16}{}\doxyparagraph{Arranging your matrices.}\label{benchmarking_ginkgo_autotoc_md16}
To allow for benchmarking of the batched functionality in Ginkgo, you need to arrange the matrix in the following fashion\+:

Store path to matrices in \$\{B\+A\+T\+C\+H\+\_\+\+M\+A\+T\+R\+I\+X\+\_\+\+F\+O\+L\+D\+ER\}


\begin{DoxyCode}{0}
\DoxyCodeLine{matrix\_class\_name}
\DoxyCodeLine{\(\backslash\)}
\DoxyCodeLine{ \(\backslash\)-\/-\/-\/0}
\DoxyCodeLine{      \(\backslash\)-\/-\/-\/A.mtx}
\DoxyCodeLine{      \(\backslash\)-\/-\/-\/b.mtx}
\DoxyCodeLine{      \(\backslash\)-\/-\/-\/S.mtx}
\DoxyCodeLine{ \(\backslash\)-\/-\/-\/1}
\DoxyCodeLine{      \(\backslash\)-\/-\/-\/A.mtx}
\DoxyCodeLine{      \(\backslash\)-\/-\/-\/b.mtx}
\DoxyCodeLine{      \(\backslash\)-\/-\/-\/S.mtx}
\DoxyCodeLine{ \(\backslash\)-\/-\/-\/2}
\DoxyCodeLine{      \(\backslash\)-\/-\/-\/A.mtx}
\DoxyCodeLine{      \(\backslash\)-\/-\/-\/b.mtx}
\DoxyCodeLine{      \(\backslash\)-\/-\/-\/S.mtx}
\DoxyCodeLine{  .}
...
\begin{DoxyItemize}
\item {\ttfamily E\+X\+E\+C\+U\+T\+OR=$<$option$>$} -\/ Select the executor to benchmark, Current supported options are\+: {\ttfamily cuda} and {\ttfamily reference}.
\item {\ttfamily B\+E\+N\+C\+H\+M\+A\+RK=batch\+\_\+solver} -\/ Benchmark the batch solvers.
\item {\ttfamily F\+O\+R\+M\+A\+TS=batch\+\_\+csr} -\/ Benchmark with the {\ttfamily Batch\+Csr} sparse matrix format.
\item {\ttfamily B\+A\+T\+C\+H\+\_\+\+S\+O\+L\+V\+E\+RS=$<$solvers$>$} -\/ The names of the batch solvers to benchmark, Options include\+: {\ttfamily bicgstab}, {\ttfamily gmres}, {\ttfamily direct}(Uses C\+U\+B\+L\+AS Dense direct). Separated by commas.
\item {\ttfamily P\+R\+E\+C\+O\+N\+DS=$<$preconditioner$>$} -\/ The preconditioner to use. Only {\ttfamily jacobi} or {\ttfamily none} are currently supported.
\item {\ttfamily N\+U\+M\+\_\+\+S\+H\+A\+R\+E\+D\+\_\+\+V\+E\+CS=$<$I$>$}-\/ The number of vectors in shared memory for the batch Bi\+C\+G\+S\+T\+AB solver. This can be reduced to allow for solution of larger system matrices and may be requried for them.
\item {\ttfamily S\+O\+L\+V\+E\+R\+S\+\_\+\+R\+HS=$<$option$>$} -\/ The R\+HS to use for the benchmarking. Options include {\ttfamily file} (Read from file, see folder structure above) and {\ttfamily 1} (all ones), {\ttfamily random} (use random values).
\item {\ttfamily S\+O\+L\+V\+E\+R\+S\+\_\+\+P\+R\+E\+C\+I\+S\+I\+ON=$<$double$>$} -\/ The tolerance to solve for the batched iterative solvers.
\item {\ttfamily S\+O\+L\+V\+E\+R\+S\+\_\+\+M\+A\+X\+\_\+\+I\+T\+E\+R\+A\+T\+I\+O\+NS=$<$int$>$} -\/ The maximum number of iterations to perform for each of the systems. Each system is independent and will iterate for these many iterations if it has not reached the convergence level.
\item {\ttfamily P\+R\+I\+N\+T\+\_\+\+R\+E\+S\+\_\+\+I\+T\+E\+RS=$<$0 or 1$>$} -\/ Whether to print the residuals, norms and errors.
\item {\ttfamily D\+E\+T\+A\+I\+L\+ED=$<$0 or 1$>$} -\/ Whether to run and gather detailed information. This prints the timings for some individual kernel timings.
\item {\ttfamily B\+A\+T\+C\+H\+\_\+\+S\+C\+A\+L\+I\+NG=$<$option$>$} -\/ Whether to use a pre-\/scaling to improve the conditioning of the matrix for the solve. Options include {\ttfamily explicit} (read from file), {\ttfamily implicit} (look for {\ttfamily A\+\_\+scaled.\+mtx}) or {\ttfamily none}.
\item {\ttfamily B\+A\+T\+C\+H\+\_\+\+M\+A\+T\+R\+I\+X\+\_\+\+L\+I\+ST=$<$filepath$>$} -\/ A text file containing the class names to be benchmarked.
\item {\ttfamily B\+A\+T\+C\+H\+\_\+\+M\+A\+T\+R\+I\+X\+\_\+\+F\+O\+L\+D\+ER=$<$foldername$>$} -\/ The folder containing the matrix classes arranged in a structure as shown above.
\item {\ttfamily N\+U\+M\+\_\+\+B\+A\+T\+C\+H\+\_\+\+D\+UP=$<$int$>$} -\/ Number of times to duplicate the provided batch systems. For example, if the {\ttfamily B\+A\+T\+C\+H\+\_\+\+M\+A\+T\+R\+I\+X\+\_\+\+F\+O\+L\+D\+ER} contained 20 matrices, and {\ttfamily N\+U\+M\+\_\+\+B\+A\+T\+C\+H\+\_\+\+D\+UP} was set to 3, then the total number of matrices would be equal to 60. This allows for performing scaling tests.
\item {\ttfamily U\+S\+E\+\_\+\+S\+U\+I\+T\+E\+\_\+\+S\+P\+A\+R\+SE=$<$0 or 1$>$} -\/ Whether to use matrices from Suite\+Sparse instead. In this case, the suite sparse matrix will be downloaded (ssget must be available) and used for individual batches (if duplication is set.)
\end{DoxyItemize}\hypertarget{benchmarking_ginkgo_autotoc_md17}{}\doxysubsection{4\+: Publishing the results on Github and analyze the results with the G\+P\+E (optional)}\label{benchmarking_ginkgo_autotoc_md17}
...
\end{DoxyEnumerate}\hypertarget{benchmarking_ginkgo_autotoc_md18}{}\doxysubsection{5\+: Detailed performance analysis and debugging}\label{benchmarking_ginkgo_autotoc_md18}
...
These features are implemented thanks to the loggers located in the file {\ttfamily \$\{ginkgo\+\_\+src\+\_\+dir\}/benchmark/utils/loggers.hpp}. Ginkgo possesses hooks at all important code location points which can be inspected thanks to the logger. In this fashion, it is easy to use these loggers also for tracking memory allocation sizes and other important library aspects.\hypertarget{benchmarking_ginkgo_autotoc_md19}{}\doxysubsection{6\+: Available benchmark options}\label{benchmarking_ginkgo_autotoc_md19}
