Using and interfacing with a batched solver..
 ...
 \label{_batched_solver_Intro}%
 \label{_batched_solver_Usingbatchedsolvers}%
...
A \char`\"{}batch\char`\"{} here means a set of small linear systems that can be solved independently, but each system is too small to use an entire computing device. A requirement is that all the systems need to have the same sparsity pattern. \label{_batched_solver_CommProg}%
 \doxysection*{The commented program}

\label{_batched_solver_Includefiles}%
...
\label{_batched_solver_Typealiasesforconvenience}%
...
\label{_batched_solver_Conveniencefunctions}%
 \doxysubsubsection*{Convenience functions}
...
\label{_batched_solver_Applicationstructuresandfunctions}%
...
We use raw pointers below to demonstrate how to handle the situation when the application only gives us raw pointers. Ideally, one should use Ginkgo\textquotesingle{}s \mbox{\hyperlink{classgko_1_1array}{gko\+::array}} class here. In this example, we assume that the data is in a format that can directly be given to a batch\+::matrix\+::\+Csr object.
...
\label{_batched_solver_Wheredoyouwanttorunyoursolver}%
...
\label{_batched_solver_Generatedata}%
...
\label{_batched_solver_Useofapplicationallocatedmemory}%
...
\label{_batched_solver_RHSandsolutionvectors}%
...
\label{_batched_solver_Createthebatchsolverfactory}%
...
\label{_batched_solver_Batchlogger}%
...
\label{_batched_solver_Generateandsolve}%
...
\label{_batched_solver_Checkresult}%
...
\label{_batched_solver_Commentsaboutprogramminganddebugging}%
\doxysubsubsection*{Comments about programming and debugging }

\label{_batched_solver_PlainProg}%
