...
This example depends on simple-\/solver.
 ...
 \label{_mixed_multigrid_solver_Intro}%
 \label{_mixed_multigrid_solver_Thisexampleshowshowtousethemixedprecisionmultigridsolver}%
...
\label{_mixed_multigrid_solver_CommProg}%
...
\DoxyCodeLine{        .with\_solver(bj::build().with\_max\_block\_size(1u))}
...
\DoxyCodeLine{        .with\_solver(bj2::build().with\_max\_block\_size(1u))}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} \mbox{\hyperlink{namespacegko_1_1log_a15f71654c6c4f4defc95f9359343fa27a7cbf673200498a0d1324f4330825202a}{solver}} = multigrid\_gen-\/>generate(A);}
...
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Multigrid execution time per iteration[ms]: "}}
\DoxyCodeLine{              << static\_cast<double>(time.count()) / 1000000.0 /}
\DoxyCodeLine{                     logger-\/>get\_num\_iterations()}
\DoxyCodeLine{              << std::endl;}
\DoxyCodeLine{\}}
\end{DoxyCode}
 \label{_mixed_multigrid_solver_Results}%
\doxysection*{Results}
...
\DoxyCodeLine{Multigrid execution time per iteration[ms]: 1.11644}
\end{DoxyCode}


\label{_mixed_multigrid_solver_Commentsaboutprogramminganddebugging}%
\doxysubsubsection*{Comments about programming and debugging }

\label{_mixed_multigrid_solver_PlainProg}%
