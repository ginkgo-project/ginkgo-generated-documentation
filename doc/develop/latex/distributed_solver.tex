...
This example depends on simple-\/solver, three-\/pt-\/stencil-\/solver.
 ...
 \label{_distributed_solver_Intro}%
 \label{_distributed_solver_Introduction}%
...
If you are using G\+PU devices, please make sure that you run this example with at most as many processes as you have G\+PU devices available. \label{_distributed_solver_CommProg}%
 \doxysection*{The commented program}

\label{_distributed_solver_Includefiles}%
...
\label{_distributed_solver_InitializetheMPIenvironment}%
...
\label{_distributed_solver_TypeDefinitions}%
...
\label{_distributed_solver_UserInputHandling}%
...
\item One-\/level, two-\/level preconditioner, and no preconditioner, defaults to one-\/level.
\end{DoxyItemize}
...
\DoxyCodeLine{                     \textcolor{stringliteral}{"[schwarz\_prec\_type] "}}
...
\DoxyCodeLine{std::string schw\_type = argc >= 5 ? argv[4] : \textcolor{stringliteral}{"one-\/level"};}
\DoxyCodeLine{}
...
\label{_distributed_solver_CreatingtheDistributedMatrixandVectors}%
...
Assemble the matrix using a 3-\/pt stencil and fill the right-\/hand-\/side with a sine value. The distributed matrix supports only constructing an empty matrix of zero size and filling in the values with \mbox{\hyperlink{classgko_1_1experimental_1_1distributed_1_1Matrix_a3e7062c96607f0d1efdbf1dff23474db}{gko\+::experimental\+::distributed\+::\+Matrix\+::read\+\_\+distributed}}. Only the data that belongs to the rows by this rank will be assembled.
...
\label{_distributed_solver_SolvetheDistributedSystem}%
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} coarse\_solver = \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(}
\DoxyCodeLine{    solver::build()}
\DoxyCodeLine{        .with\_preconditioner(}
\DoxyCodeLine{            schwarz::build().with\_local\_solver(local\_solver).on(exec))}
...
\DoxyCodeLine{                .with\_reduction\_factor(1e-\/7)}
\DoxyCodeLine{                .on(exec))}
\DoxyCodeLine{        .on(exec));}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{keyword}{auto} pgm\_fac = \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{gko::share}}(pgm::build().on(exec));}
\end{DoxyCode}
...
\DoxyCodeLine{std::shared\_ptr<gko::LinOp> Ainv\{\};}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keywordflow}{if} (schw\_type == \textcolor{stringliteral}{"two-\/level"}) \{}
\DoxyCodeLine{    Ainv =}
\DoxyCodeLine{        solver::build()}
\DoxyCodeLine{            .with\_preconditioner(schwarz::build()}
\DoxyCodeLine{                                     .with\_local\_solver(local\_solver)}
\DoxyCodeLine{                                     .with\_coarse\_level(pgm\_fac)}
\DoxyCodeLine{                                     .with\_coarse\_solver(coarse\_solver)}
\DoxyCodeLine{                                     .on(exec))}
\DoxyCodeLine{            .with\_criteria(}
...
\DoxyCodeLine{\} \textcolor{keywordflow}{else} \textcolor{keywordflow}{if} (schw\_type == \textcolor{stringliteral}{"one-\/level"}) \{}
\DoxyCodeLine{    Ainv =}
\DoxyCodeLine{        solver::build()}
\DoxyCodeLine{            .with\_preconditioner(}
\DoxyCodeLine{                schwarz::build().with\_local\_solver(local\_solver).on(exec))}
...
\DoxyCodeLine{            -\/>generate(A);}
\DoxyCodeLine{\} \textcolor{keywordflow}{else} \{}
\DoxyCodeLine{    schw\_type = \textcolor{stringliteral}{"no-\/precond"};}
\DoxyCodeLine{    Ainv =}
\DoxyCodeLine{        solver::build()}
\DoxyCodeLine{            .with\_criteria(}
\DoxyCodeLine{                gko::stop::Iteration::build().with\_max\_iters(num\_iters).on(}
...
\DoxyCodeLine{\textcolor{keyword}{auto} host\_res = \mbox{\hyperlink{namespacegko_a7e65327b79b773f00f5bf4a480dd2066}{gko::make\_temporary\_clone}}(exec-\/>get\_master(), res\_norm);}
\end{DoxyCode}


\label{_distributed_solver_PrintingResults}%
...
\DoxyCodeLine{          << \textcolor{stringliteral}{"\(\backslash\)nPrecond type: "} << schw\_type}
\DoxyCodeLine{          << \textcolor{stringliteral}{"\(\backslash\)nFinal Res norm: "} << *host\_res-\/>get\_const\_values()}
...
\DoxyCodeLine{\}}
\end{DoxyCode}
 \label{_distributed_solver_Results}%
\doxysection*{Results}
...
The timings may vary depending on the machine. \label{_distributed_solver_PlainProg}%
