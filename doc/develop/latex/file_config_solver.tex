The file config solver example..

This example depends on simple-\/solver.

 \label{_file_config_solver_Intro}%
 \label{_file_config_solver_Thisexampleshowshowtousefiletoconfiguresolver}%
\doxysection*{This example shows how to use file to configure solver.}

\label{_file_config_solver_InthisexamplewefirstreadinamatrixfromafileThenwereadtheconfigfiletoconfigurethesolverTheexampleapplicationreportsthegenerationandruntimeofthesolver}%
\doxysubsubsection*{In this example, we first read in a matrix from a file. Then we read the config file to configure the solver. The example application reports the generation and run time of the solver.}

\label{_file_config_solver_CommProg}%
 \doxysection*{The commented program}


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <chrono>}}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <fstream>}}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <iomanip>}}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <iostream>}}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <map>}}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <string>}}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <ginkgo/ginkgo.hpp>}}
\end{DoxyCode}


the header in extensions is not shipped with \mbox{\hyperlink{ginkgo_8hpp_source}{ginkgo.\+hpp}}


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <ginkgo/extensions/config/json\_config.hpp>}}
\DoxyCodeLine{}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{keywordtype}{int} main(\textcolor{keywordtype}{int} argc, \textcolor{keywordtype}{char}* argv[])}
\DoxyCodeLine{\{}
\end{DoxyCode}


Some shortcuts


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{using} ValueType = double;}
\DoxyCodeLine{\textcolor{keyword}{using} IndexType = int;}
\DoxyCodeLine{\textcolor{keyword}{using} vec = \mbox{\hyperlink{classgko_1_1matrix_1_1Dense}{gko::matrix::Dense<ValueType>}};}
\DoxyCodeLine{\textcolor{keyword}{using} mtx = \mbox{\hyperlink{classgko_1_1matrix_1_1Csr}{gko::matrix::Csr<ValueType, IndexType>}};}
\end{DoxyCode}


Print version information


\begin{DoxyCode}{0}
\DoxyCodeLine{std::cout << \mbox{\hyperlink{classgko_1_1version__info_a6daeb8a087cfb57fa055526fc133d8eb}{gko::version\_info::get}}() << std::endl;}
\end{DoxyCode}


Print usage


\begin{DoxyCode}{0}
\DoxyCodeLine{std::cout << argv[0] << \textcolor{stringliteral}{" executor configfile matrix"} << std::endl;}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keyword}{auto} executor\_string = argc >= 2 ? argv[1] : \textcolor{stringliteral}{"reference"};}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keyword}{auto} configfile = argc >= 3 ? argv[2] : \textcolor{stringliteral}{"config/cg.json"};}
\DoxyCodeLine{\textcolor{keyword}{const} std::string matrix\_path = argc >= 4 ? argv[3] : \textcolor{stringliteral}{"data/A.mtx"};}
\end{DoxyCode}


Figure out where to run the code


\begin{DoxyCode}{0}
\DoxyCodeLine{std::map<std::string, std::function<std::shared\_ptr<gko::Executor>()>>}
\DoxyCodeLine{    exec\_map\{}
\DoxyCodeLine{        \{\textcolor{stringliteral}{"omp"}, [] \{ \textcolor{keywordflow}{return} \mbox{\hyperlink{classgko_1_1OmpExecutor_a8930142a1913bcbcac4fb6c917f930d9}{gko::OmpExecutor::create}}(); \}\},}
\DoxyCodeLine{        \{\textcolor{stringliteral}{"cuda"},}
\DoxyCodeLine{         [] \{}
\DoxyCodeLine{             \textcolor{keywordflow}{return} \mbox{\hyperlink{classgko_1_1CudaExecutor_a6c423656c7c0eb53810cb77bb4a66d15}{gko::CudaExecutor::create}}(0,}
\DoxyCodeLine{                                              \mbox{\hyperlink{classgko_1_1OmpExecutor_a8930142a1913bcbcac4fb6c917f930d9}{gko::OmpExecutor::create}}());}
\DoxyCodeLine{         \}\},}
\DoxyCodeLine{        \{\textcolor{stringliteral}{"hip"},}
\DoxyCodeLine{         [] \{}
\DoxyCodeLine{             \textcolor{keywordflow}{return} \mbox{\hyperlink{classgko_1_1HipExecutor_aa0701fe1303e03a8331da7e95008100d}{gko::HipExecutor::create}}(0, \mbox{\hyperlink{classgko_1_1OmpExecutor_a8930142a1913bcbcac4fb6c917f930d9}{gko::OmpExecutor::create}}());}
\DoxyCodeLine{         \}\},}
\DoxyCodeLine{        \{\textcolor{stringliteral}{"dpcpp"},}
\DoxyCodeLine{         [] \{}
\DoxyCodeLine{             \textcolor{keywordflow}{return} \mbox{\hyperlink{classgko_1_1DpcppExecutor_ae9b875dfe3af7f3f3f24071bfb2f895e}{gko::DpcppExecutor::create}}(}
\DoxyCodeLine{                 0, gko::ReferenceExecutor::create());}
\DoxyCodeLine{         \}\},}
\DoxyCodeLine{        \{\textcolor{stringliteral}{"reference"}, [] \{ \textcolor{keywordflow}{return} gko::ReferenceExecutor::create(); \}\}\};}
\end{DoxyCode}


executor where Ginkgo will perform the computation


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keyword}{auto} exec = exec\_map.at(executor\_string)();  \textcolor{comment}{// throws if not valid}}
\end{DoxyCode}


Read data


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} A = \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{share}}(gko::read<mtx>(std::ifstream(matrix\_path), exec));}
\end{DoxyCode}


Create R\+HS as 1 and initial guess as 0


\begin{DoxyCode}{0}
\DoxyCodeLine{\mbox{\hyperlink{namespacegko_a6e5c95df0ae4e47aab2f604a22d98ee7}{gko::size\_type}} size = A-\/>get\_size()[0];}
\DoxyCodeLine{\textcolor{keyword}{auto} host\_x = vec::create(exec-\/>get\_master(), \mbox{\hyperlink{structgko_1_1dim}{gko::dim<2>}}(size, 1));}
\DoxyCodeLine{\textcolor{keyword}{auto} host\_b = vec::create(exec-\/>get\_master(), \mbox{\hyperlink{structgko_1_1dim}{gko::dim<2>}}(size, 1));}
\DoxyCodeLine{\textcolor{keywordflow}{for} (\textcolor{keyword}{auto} i = 0; i < size; i++) \{}
\DoxyCodeLine{    host\_x-\/>at(i, 0) = 0.;}
\DoxyCodeLine{    host\_b-\/>at(i, 0) = 1.;}
\DoxyCodeLine{\}}
\DoxyCodeLine{\textcolor{keyword}{auto} x = vec::create(exec);}
\DoxyCodeLine{\textcolor{keyword}{auto} b = vec::create(exec);}
\DoxyCodeLine{x-\/>copy\_from(host\_x);}
\DoxyCodeLine{b-\/>copy\_from(host\_b);}
\end{DoxyCode}


Calculate initial residual by overwriting b


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} \mbox{\hyperlink{namespacegko_a0059e27f8f4bc348ff65c1e60caf47c8}{one}} = gko::initialize<vec>(\{1.0\}, exec);}
\DoxyCodeLine{\textcolor{keyword}{auto} neg\_one = gko::initialize<vec>(\{-\/1.0\}, exec);}
\DoxyCodeLine{\textcolor{keyword}{auto} initres = gko::initialize<vec>(\{0.0\}, exec);}
\DoxyCodeLine{A-\/>apply(one, x, neg\_one, b);}
\DoxyCodeLine{b-\/>compute\_norm2(initres);}
\end{DoxyCode}


Copy b again


\begin{DoxyCode}{0}
\DoxyCodeLine{b-\/>copy\_from(host\_b);}
\end{DoxyCode}


Read the json config file to configure the ginkgo solver. The following files, which are mapped to corresponding examples, are available cg.\+json\+: simple-\/solver blockjacobi-\/cg.\+json\+: preconditioned-\/solver ir.\+json\+: iterative-\/refinement parilu.\+json\+: ilu-\/preconditioned-\/solver (by using factorization parameter directly) Ilu only supports the value\+\_\+type variant for parse. pgm-\/multigrid-\/cg.\+json\+: multigrid-\/preconditioned-\/solver (set min\+\_\+coarse\+\_\+rows additionally due to this small example matrix) mixed-\/pgm-\/multigrid-\/cg.\+json\+: mixed-\/multigrid-\/preconditioned-\/solver (assuming there are always more than one level)


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} config = gko::ext::config::parse\_json\_file(configfile);}
\end{DoxyCode}


Create the registry, which allows passing the existing data into config This example does not use existing data.


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} reg = \mbox{\hyperlink{classgko_1_1config_1_1registry}{gko::config::registry}}();}
\end{DoxyCode}


Create the default type descriptor, which gives the default common type (value/index) for solver generation. If the solver does not specify value type, the solver will use these types.


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} td = gko::config::make\_type\_descriptor<ValueType, IndexType>();}
\end{DoxyCode}


generate the linopfactory on the given executors


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} solver\_gen = gko::config::parse(config, reg, td).on(exec);}
\end{DoxyCode}


Create solver


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keyword}{auto} gen\_tic = std::chrono::steady\_clock::now();}
\DoxyCodeLine{\textcolor{keyword}{auto} \mbox{\hyperlink{namespacegko_1_1log_a15f71654c6c4f4defc95f9359343fa27a7cbf673200498a0d1324f4330825202a}{solver}} = solver\_gen-\/>generate(A);}
\DoxyCodeLine{exec-\/>synchronize();}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keyword}{auto} gen\_toc = std::chrono::steady\_clock::now();}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keyword}{auto} gen\_time = std::chrono::duration\_cast<std::chrono::milliseconds>(}
\DoxyCodeLine{    gen\_toc -\/ gen\_tic);}
\end{DoxyCode}


Add logger


\begin{DoxyCode}{0}
\DoxyCodeLine{std::shared\_ptr<const gko::log::Convergence<ValueType>> logger =}
\DoxyCodeLine{    \mbox{\hyperlink{classgko_1_1log_1_1Convergence_aa67955ea9d1d18f15f0330d12b93d330}{gko::log::Convergence<ValueType>::create}}();}
\DoxyCodeLine{\mbox{\hyperlink{namespacegko_1_1log_a15f71654c6c4f4defc95f9359343fa27a7cbf673200498a0d1324f4330825202a}{solver}}-\/>add\_logger(logger);}
\end{DoxyCode}


Solve system


\begin{DoxyCode}{0}
\DoxyCodeLine{exec-\/>synchronize();}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keyword}{auto} tic = std::chrono::steady\_clock::now();}
\DoxyCodeLine{\mbox{\hyperlink{namespacegko_1_1log_a15f71654c6c4f4defc95f9359343fa27a7cbf673200498a0d1324f4330825202a}{solver}}-\/>apply(b, x);}
\DoxyCodeLine{exec-\/>synchronize();}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keyword}{auto} toc = std::chrono::steady\_clock::now();}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keyword}{auto} time =}
\DoxyCodeLine{    std::chrono::duration\_cast<std::chrono::milliseconds>(toc -\/ tic);}
\end{DoxyCode}


Print out the solver config


\begin{DoxyCode}{0}
\DoxyCodeLine{std::cout << \textcolor{stringliteral}{"Config file: "} << configfile << std::endl;}
\DoxyCodeLine{std::ifstream f(configfile);}
\DoxyCodeLine{std::cout << f.rdbuf() << std::endl;}
\end{DoxyCode}


Calculate residual


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} res = gko::as<vec>(logger-\/>get\_residual\_norm());}
\DoxyCodeLine{}
\DoxyCodeLine{std::cout << \textcolor{stringliteral}{"Initial residual norm sqrt(r\string^T r): \(\backslash\)n"};}
\DoxyCodeLine{\mbox{\hyperlink{namespacegko_a577a2f0345ee607f64277b3364b6431b}{write}}(std::cout, initres);}
\DoxyCodeLine{std::cout << \textcolor{stringliteral}{"Final residual norm sqrt(r\string^T r): \(\backslash\)n"};}
\DoxyCodeLine{\mbox{\hyperlink{namespacegko_a577a2f0345ee607f64277b3364b6431b}{write}}(std::cout, res);}
\end{DoxyCode}


Print solver statistics


\begin{DoxyCode}{0}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Solver iteration count:     "} << logger-\/>get\_num\_iterations()}
\DoxyCodeLine{              << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Solver generation time [ms]: "}}
\DoxyCodeLine{              << static\_cast<double>(gen\_time.count()) << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Solver execution time [ms]: "}}
\DoxyCodeLine{              << static\_cast<double>(time.count()) << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Solver execution time per iteration[ms]: "}}
\DoxyCodeLine{              << static\_cast<double>(time.count()) /}
\DoxyCodeLine{                     logger-\/>get\_num\_iterations()}
\DoxyCodeLine{              << std::endl;}
\DoxyCodeLine{\}}
\end{DoxyCode}
 \label{_file_config_solver_Results}%
\doxysection*{Results}

This is the expected output\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{Config file: config/cg.json}
\DoxyCodeLine{\{}
\DoxyCodeLine{    \textcolor{stringliteral}{"type"}: \textcolor{stringliteral}{"solver::Cg"},}
\DoxyCodeLine{    \textcolor{stringliteral}{"criteria"}: [}
\DoxyCodeLine{        \{}
\DoxyCodeLine{            \textcolor{stringliteral}{"type"}: \textcolor{stringliteral}{"Iteration"},}
\DoxyCodeLine{            \textcolor{stringliteral}{"max\_iters"}: 20}
\DoxyCodeLine{        \},}
\DoxyCodeLine{        \{}
\DoxyCodeLine{            \textcolor{stringliteral}{"type"}: \textcolor{stringliteral}{"ResidualNorm"},}
\DoxyCodeLine{            \textcolor{stringliteral}{"reduction\_factor"}: 1e-\/7}
\DoxyCodeLine{        \}}
\DoxyCodeLine{    ]}
\DoxyCodeLine{\}}
\DoxyCodeLine{Initial residual norm sqrt(r\string^T r):}
\DoxyCodeLine{\%\%MatrixMarket matrix \mbox{\hyperlink{namespacegko_ae749a5ea11a93c1bcc9158d9a6e9fb68af1f713c9e000f5d3f280adbd124df4f5}{array}} \mbox{\hyperlink{namespacegko_af21648097306adbabc6234b4609ce297}{real}} general}
\DoxyCodeLine{1 1}
\DoxyCodeLine{25.9808}
\DoxyCodeLine{Final residual norm sqrt(r\string^T r):}
\DoxyCodeLine{\%\%MatrixMarket matrix \mbox{\hyperlink{namespacegko_ae749a5ea11a93c1bcc9158d9a6e9fb68af1f713c9e000f5d3f280adbd124df4f5}{array}} \mbox{\hyperlink{namespacegko_af21648097306adbabc6234b4609ce297}{real}} general}
\DoxyCodeLine{1 1}
\DoxyCodeLine{58.79}
\DoxyCodeLine{Solver iteration count:     20}
\DoxyCodeLine{Solver generation time [ms]: 0.065244}
\DoxyCodeLine{Solver execution time [ms]: 0.793764}
\DoxyCodeLine{Solver execution time per iteration[ms]: 0.0396882}
\end{DoxyCode}


\label{_file_config_solver_Commentsaboutprogramminganddebugging}%
\doxysubsubsection*{Comments about programming and debugging }

\label{_file_config_solver_PlainProg}%
 \doxysection*{The plain program}


\begin{DoxyCodeInclude}{0}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <chrono>}}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <fstream>}}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <iomanip>}}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <iostream>}}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <map>}}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <string>}}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <ginkgo/ginkgo.hpp>}}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{preprocessor}{\#include <ginkgo/extensions/config/json\_config.hpp>}}
\DoxyCodeLine{}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{keywordtype}{int} main(\textcolor{keywordtype}{int} argc, \textcolor{keywordtype}{char}* argv[])}
\DoxyCodeLine{\{}
\DoxyCodeLine{    \textcolor{keyword}{using} ValueType = double;}
\DoxyCodeLine{    \textcolor{keyword}{using} IndexType = int;}
\DoxyCodeLine{    \textcolor{keyword}{using} vec = \mbox{\hyperlink{classgko_1_1matrix_1_1Dense}{gko::matrix::Dense<ValueType>}};}
\DoxyCodeLine{    \textcolor{keyword}{using} mtx = \mbox{\hyperlink{classgko_1_1matrix_1_1Csr}{gko::matrix::Csr<ValueType, IndexType>}};}
\DoxyCodeLine{}
\DoxyCodeLine{    std::cout << \mbox{\hyperlink{classgko_1_1version__info_a6daeb8a087cfb57fa055526fc133d8eb}{gko::version\_info::get}}() << std::endl;}
\DoxyCodeLine{    std::cout << argv[0] << \textcolor{stringliteral}{" executor configfile matrix"} << std::endl;}
\DoxyCodeLine{}
\DoxyCodeLine{    \textcolor{keyword}{const} \textcolor{keyword}{auto} executor\_string = argc >= 2 ? argv[1] : \textcolor{stringliteral}{"reference"};}
\DoxyCodeLine{    \textcolor{keyword}{const} \textcolor{keyword}{auto} configfile = argc >= 3 ? argv[2] : \textcolor{stringliteral}{"config/cg.json"};}
\DoxyCodeLine{    \textcolor{keyword}{const} std::string matrix\_path = argc >= 4 ? argv[3] : \textcolor{stringliteral}{"data/A.mtx"};}
\DoxyCodeLine{}
\DoxyCodeLine{    std::map<std::string, std::function<std::shared\_ptr<gko::Executor>()>>}
\DoxyCodeLine{        exec\_map\{}
\DoxyCodeLine{            \{\textcolor{stringliteral}{"omp"}, [] \{ \textcolor{keywordflow}{return} \mbox{\hyperlink{classgko_1_1OmpExecutor_a8930142a1913bcbcac4fb6c917f930d9}{gko::OmpExecutor::create}}(); \}\},}
\DoxyCodeLine{            \{\textcolor{stringliteral}{"cuda"},}
\DoxyCodeLine{             [] \{}
\DoxyCodeLine{                 \textcolor{keywordflow}{return} \mbox{\hyperlink{classgko_1_1CudaExecutor_a6c423656c7c0eb53810cb77bb4a66d15}{gko::CudaExecutor::create}}(0,}
\DoxyCodeLine{                                                  \mbox{\hyperlink{classgko_1_1OmpExecutor_a8930142a1913bcbcac4fb6c917f930d9}{gko::OmpExecutor::create}}());}
\DoxyCodeLine{             \}\},}
\DoxyCodeLine{            \{\textcolor{stringliteral}{"hip"},}
\DoxyCodeLine{             [] \{}
\DoxyCodeLine{                 \textcolor{keywordflow}{return} \mbox{\hyperlink{classgko_1_1HipExecutor_aa0701fe1303e03a8331da7e95008100d}{gko::HipExecutor::create}}(0, \mbox{\hyperlink{classgko_1_1OmpExecutor_a8930142a1913bcbcac4fb6c917f930d9}{gko::OmpExecutor::create}}());}
\DoxyCodeLine{             \}\},}
\DoxyCodeLine{            \{\textcolor{stringliteral}{"dpcpp"},}
\DoxyCodeLine{             [] \{}
\DoxyCodeLine{                 \textcolor{keywordflow}{return} \mbox{\hyperlink{classgko_1_1DpcppExecutor_ae9b875dfe3af7f3f3f24071bfb2f895e}{gko::DpcppExecutor::create}}(}
\DoxyCodeLine{                     0, gko::ReferenceExecutor::create());}
\DoxyCodeLine{             \}\},}
\DoxyCodeLine{            \{\textcolor{stringliteral}{"reference"}, [] \{ \textcolor{keywordflow}{return} gko::ReferenceExecutor::create(); \}\}\};}
\DoxyCodeLine{}
\DoxyCodeLine{    \textcolor{keyword}{const} \textcolor{keyword}{auto} exec = exec\_map.at(executor\_string)();  \textcolor{comment}{// throws if not valid}}
\DoxyCodeLine{}
\DoxyCodeLine{    \textcolor{keyword}{auto} A = \mbox{\hyperlink{namespacegko_a3ce296f73db0ff398bdea6009a3a5c58}{share}}(gko::read<mtx>(std::ifstream(matrix\_path), exec));}
\DoxyCodeLine{    \mbox{\hyperlink{namespacegko_a6e5c95df0ae4e47aab2f604a22d98ee7}{gko::size\_type}} size = A-\/>get\_size()[0];}
\DoxyCodeLine{    \textcolor{keyword}{auto} host\_x = vec::create(exec-\/>get\_master(), \mbox{\hyperlink{structgko_1_1dim}{gko::dim<2>}}(size, 1));}
\DoxyCodeLine{    \textcolor{keyword}{auto} host\_b = vec::create(exec-\/>get\_master(), \mbox{\hyperlink{structgko_1_1dim}{gko::dim<2>}}(size, 1));}
\DoxyCodeLine{    \textcolor{keywordflow}{for} (\textcolor{keyword}{auto} i = 0; i < size; i++) \{}
\DoxyCodeLine{        host\_x-\/>at(i, 0) = 0.;}
\DoxyCodeLine{        host\_b-\/>at(i, 0) = 1.;}
\DoxyCodeLine{    \}}
\DoxyCodeLine{    \textcolor{keyword}{auto} x = vec::create(exec);}
\DoxyCodeLine{    \textcolor{keyword}{auto} b = vec::create(exec);}
\DoxyCodeLine{    x-\/>copy\_from(host\_x);}
\DoxyCodeLine{    b-\/>copy\_from(host\_b);}
\DoxyCodeLine{}
\DoxyCodeLine{    \textcolor{keyword}{auto} \mbox{\hyperlink{namespacegko_a0059e27f8f4bc348ff65c1e60caf47c8}{one}} = gko::initialize<vec>(\{1.0\}, exec);}
\DoxyCodeLine{    \textcolor{keyword}{auto} neg\_one = gko::initialize<vec>(\{-\/1.0\}, exec);}
\DoxyCodeLine{    \textcolor{keyword}{auto} initres = gko::initialize<vec>(\{0.0\}, exec);}
\DoxyCodeLine{    A-\/>apply(one, x, neg\_one, b);}
\DoxyCodeLine{    b-\/>compute\_norm2(initres);}
\DoxyCodeLine{}
\DoxyCodeLine{    b-\/>copy\_from(host\_b);}
\DoxyCodeLine{}
\DoxyCodeLine{    \textcolor{keyword}{auto} config = gko::ext::config::parse\_json\_file(configfile);}
\DoxyCodeLine{    \textcolor{keyword}{auto} reg = \mbox{\hyperlink{classgko_1_1config_1_1registry}{gko::config::registry}}();}
\DoxyCodeLine{    \textcolor{keyword}{auto} td = gko::config::make\_type\_descriptor<ValueType, IndexType>();}
\DoxyCodeLine{    \textcolor{keyword}{auto} solver\_gen = gko::config::parse(config, reg, td).on(exec);}
\DoxyCodeLine{}
\DoxyCodeLine{    \textcolor{keyword}{const} \textcolor{keyword}{auto} gen\_tic = std::chrono::steady\_clock::now();}
\DoxyCodeLine{    \textcolor{keyword}{auto} \mbox{\hyperlink{namespacegko_1_1log_a15f71654c6c4f4defc95f9359343fa27a7cbf673200498a0d1324f4330825202a}{solver}} = solver\_gen-\/>generate(A);}
\DoxyCodeLine{    exec-\/>synchronize();}
\DoxyCodeLine{    \textcolor{keyword}{const} \textcolor{keyword}{auto} gen\_toc = std::chrono::steady\_clock::now();}
\DoxyCodeLine{    \textcolor{keyword}{const} \textcolor{keyword}{auto} gen\_time = std::chrono::duration\_cast<std::chrono::milliseconds>(}
\DoxyCodeLine{        gen\_toc -\/ gen\_tic);}
\DoxyCodeLine{}
\DoxyCodeLine{    std::shared\_ptr<const gko::log::Convergence<ValueType>> logger =}
\DoxyCodeLine{        \mbox{\hyperlink{classgko_1_1log_1_1Convergence_aa67955ea9d1d18f15f0330d12b93d330}{gko::log::Convergence<ValueType>::create}}();}
\DoxyCodeLine{    \mbox{\hyperlink{namespacegko_1_1log_a15f71654c6c4f4defc95f9359343fa27a7cbf673200498a0d1324f4330825202a}{solver}}-\/>add\_logger(logger);}
\DoxyCodeLine{}
\DoxyCodeLine{    exec-\/>synchronize();}
\DoxyCodeLine{    \textcolor{keyword}{const} \textcolor{keyword}{auto} tic = std::chrono::steady\_clock::now();}
\DoxyCodeLine{    \mbox{\hyperlink{namespacegko_1_1log_a15f71654c6c4f4defc95f9359343fa27a7cbf673200498a0d1324f4330825202a}{solver}}-\/>apply(b, x);}
\DoxyCodeLine{    exec-\/>synchronize();}
\DoxyCodeLine{    \textcolor{keyword}{const} \textcolor{keyword}{auto} toc = std::chrono::steady\_clock::now();}
\DoxyCodeLine{    \textcolor{keyword}{const} \textcolor{keyword}{auto} time =}
\DoxyCodeLine{        std::chrono::duration\_cast<std::chrono::milliseconds>(toc -\/ tic);}
\DoxyCodeLine{}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Config file: "} << configfile << std::endl;}
\DoxyCodeLine{    std::ifstream f(configfile);}
\DoxyCodeLine{    std::cout << f.rdbuf() << std::endl;}
\DoxyCodeLine{}
\DoxyCodeLine{    \textcolor{keyword}{auto} res = gko::as<vec>(logger-\/>get\_residual\_norm());}
\DoxyCodeLine{}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Initial residual norm sqrt(r\string^T r): \(\backslash\)n"};}
\DoxyCodeLine{    \mbox{\hyperlink{namespacegko_a577a2f0345ee607f64277b3364b6431b}{write}}(std::cout, initres);}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Final residual norm sqrt(r\string^T r): \(\backslash\)n"};}
\DoxyCodeLine{    \mbox{\hyperlink{namespacegko_a577a2f0345ee607f64277b3364b6431b}{write}}(std::cout, res);}
\DoxyCodeLine{}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Solver iteration count:     "} << logger-\/>get\_num\_iterations()}
\DoxyCodeLine{              << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Solver generation time [ms]: "}}
\DoxyCodeLine{              << static\_cast<double>(gen\_time.count()) << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Solver execution time [ms]: "}}
\DoxyCodeLine{              << static\_cast<double>(time.count()) << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Solver execution time per iteration[ms]: "}}
\DoxyCodeLine{              << static\_cast<double>(time.count()) /}
\DoxyCodeLine{                     logger-\/>get\_num\_iterations()}
\DoxyCodeLine{              << std::endl;}
\DoxyCodeLine{\}}
\end{DoxyCodeInclude}
 