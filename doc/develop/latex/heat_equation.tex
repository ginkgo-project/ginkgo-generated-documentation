...
This example depends on simple-\/solver, three-\/pt-\/stencil-\/solver.
 ...
 \label{_heat_equation_Intro}%
 \label{_heat_equation_Introduction}%
...
\label{_heat_equation_Abouttheexample}%
\doxysubsubsection*{About the example }

\label{_heat_equation_CommProg}%
...
\DoxyCodeLine{and grid distance \(\backslash\)fh = 1/n\(\backslash\)f we \mbox{\hyperlink{namespacegko_a577a2f0345ee607f64277b3364b6431b}{write}}}
\DoxyCodeLine{}
...
\DoxyCodeLine{\textcolor{stringliteral}{We then build an implicit Euler integrator by discretizing with time step}}
\DoxyCodeLine{\textcolor{stringliteral}{\(\backslash\)f\(\backslash\)tau\(\backslash\)f}}
\DoxyCodeLine{\textcolor{stringliteral}{}}
...
\DoxyCodeLine{\textcolor{stringliteral}{    + f\_\{i,j\}}}
\DoxyCodeLine{\textcolor{stringliteral}{}}
\DoxyCodeLine{\textcolor{stringliteral}{and solve the resulting linear system for \(\backslash\)f u\_\{\(\backslash\)cdot\}\string^\{k+1\}\(\backslash\)f using}}
\DoxyCodeLine{\textcolor{stringliteral}{Ginkgo'}s CG \mbox{\hyperlink{namespacegko_1_1log_a15f71654c6c4f4defc95f9359343fa27a7cbf673200498a0d1324f4330825202a}{solver}} preconditioned with an incomplete Cholesky factorization}
\DoxyCodeLine{\textcolor{keywordflow}{for each} time step, occasionally writing the resulting grid values into a}
\DoxyCodeLine{video file \textcolor{keyword}{using} OpenCV and a custom color mapping.}
\DoxyCodeLine{}
...
\DoxyCodeLine{\textcolor{comment}{and grid distance \(\backslash\)f\$h = 1/n\(\backslash\)f\$ we write}}
\DoxyCodeLine{\textcolor{comment}{}}
...
\DoxyCodeLine{\textcolor{comment}{}}
\DoxyCodeLine{\textcolor{comment}{We then build an implicit Euler integrator by discretizing with time step}}
\DoxyCodeLine{\textcolor{comment}{\(\backslash\)f\$\(\backslash\)tau\(\backslash\)f\$}}
\DoxyCodeLine{\textcolor{comment}{}}
...
\DoxyCodeLine{\textcolor{comment}{    + f\_\{i,j\}}}
\DoxyCodeLine{\textcolor{comment}{}}
\DoxyCodeLine{\textcolor{comment}{and solve the resulting linear system for \(\backslash\)f\$ u\_\{\(\backslash\)cdot\}\string^\{k+1\}\(\backslash\)f\$ using}}
\DoxyCodeLine{\textcolor{comment}{Ginkgo's CG solver preconditioned with an incomplete Cholesky factorization}}
\DoxyCodeLine{\textcolor{comment}{for each time step, occasionally writing the resulting grid values into a}}
\DoxyCodeLine{\textcolor{comment}{video file using OpenCV and a custom color mapping.}}
\DoxyCodeLine{\textcolor{comment}{}}
