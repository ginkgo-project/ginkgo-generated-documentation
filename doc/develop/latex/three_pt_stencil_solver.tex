...
This example depends on simple-\/solver, poisson-\/solver.
 ...
 \label{_three_pt_stencil_solver_Intro}%
 \label{_three_pt_stencil_solver_Introduction}%
...
which is then solved using Ginkgo\textquotesingle{}s implementation of the CG method preconditioned with block-\/\+Jacobi. It is also possible to specify on which executor Ginkgo will solve the system via the command line. The function {\ttfamily f} is set to {\ttfamily f(x) = 6x} (making the solution {\ttfamily u(x) = x$^\wedge$3}), but that can be changed in the {\ttfamily main} function.
...
\label{_three_pt_stencil_solver_Abouttheexample}%
\doxysubsubsection*{About the example }

\label{_three_pt_stencil_solver_CommProg}%
...
\DoxyCodeLine{\}}
\end{DoxyCode}
 \label{_three_pt_stencil_solver_Results}%
\doxysection*{Results}
...
\label{_three_pt_stencil_solver_Commentsaboutprogramminganddebugging}%
\doxysubsubsection*{Comments about programming and debugging }

\label{_three_pt_stencil_solver_PlainProg}%
