...
\begin{DoxyItemize}
\item The {\ttfamily develop} branch is the default branch to submit PR\textquotesingle{}s to. From time to time, we merge the {\ttfamily develop} branch to the {\ttfamily main} branch and create tags on the {\ttfamily main} to create new releases of Ginkgo. Therefore, all pull requests must be merged into {\ttfamily develop}.
...
Ginkgo uses \href{https://pre-commit.com/}{\texttt{ pre-\/commit}} to automatically apply code formatting when committing changes to git. What formatting is applied is managed through \href{https://clang.llvm.org/docs/ClangFormat.html}{\texttt{ Clang\+Format}} with a custom {\ttfamily .clang-\/format} configuration file (mostly based on Clang\+Format\textquotesingle{}s {\itshape Google} style). {\bfseries{Make sure you have pre-\/commit set up and running properly}} before committing anything that will end up in a pull request against {\ttfamily ginkgo-\/project/ginkgo} repository. In addition, you should {\bfseries{never}} modify the {\ttfamily .clang-\/format} configuration file shipped with Ginkgo. E.\+g. if Clang\+Format has trouble reading this file on your system, you should install a newer version of Clang\+Format, and avoid commenting out parts of the configuration file.
...
The concrete ordering will be done by {\ttfamily clang-\/format}. Here are the rules that {\ttfamily clang-\/format} will follow. In general, all include statements should be present on the top of the file, ordered in the following groups, with {\itshape one} blank lines between each group\+:


\begin{DoxyEnumerate}
\item Main header file (e.\+g. {\ttfamily core/foo/bar.\+hpp} included in {\ttfamily core/foo/bar.\+cpp}, or in the unit test{\ttfamily core/test/foo/bar.\+cpp})
...
This section presents the handling of the main header attributed to a file. For a given file, the main header is the header that contains the declarations of the functions, classes, etc., which are implemented in this file. In the previous example, this would be {\ttfamily \#include \char`\"{}ginkgo/core/base/my\+\_\+file.\+hpp\char`\"{}}. The {\ttfamily clang-\/format} tool figures out the main header. The only intervention form a contributor is to {\itshape always} include the main header using {\ttfamily \char`\"{}...\char`\"{}}.

Please note that this only applies to implementation files, so files ending in {\ttfamily .cpp} or {\ttfamily .cu}.\hypertarget{contributing_guidelines_autotoc_md43}{}\doxysubsubsection{Some general comments.}\label{contributing_guidelines_autotoc_md43}
