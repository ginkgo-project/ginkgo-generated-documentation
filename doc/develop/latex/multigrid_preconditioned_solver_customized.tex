...
\DoxyCodeLine{        .with\_factorization\_factory(}
\DoxyCodeLine{            \mbox{\hyperlink{classgko_1_1factorization_1_1Ic}{gko::factorization::Ic<ValueType, int>::build}}().on(exec))}
\DoxyCodeLine{        .on(exec));}
...
\DoxyCodeLine{CG execution time per iteraion[ms]: 0.54937}
\end{DoxyCode}
